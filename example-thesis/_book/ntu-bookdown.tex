...
Library\footnote{根據 Yihui 的說明
  (\url{https://yihui.name/en/2018/08/tinytex-flash-drive})，TinyTex
  具有可攜式（portable）的特性。}：
...
\item
  \href{http://bit.ly/TinyTeX-win7}{Windows 7 64bit}
\item
  Windows 10 64bit
  ...
  \tightlist
  \item
    目前不確定是否可直接使用\href{http://bit.ly/TinyTeX-win7}{Windows 7
    64bit}，需要使用者們的測試。若可以的話，希望可以通知我（在下方留言或寄信給我）
  \end{itemize}
\item
  Mac
...
此外，可以將安裝下來的 TinyTeX
及其套件複製到其它（同作業系統的）電腦上，如此就不需每此換電腦就要重新下載
LaTeX 套件。
...
即會跳出一個新視窗，選擇資料夾後，即可將安裝在電腦上的套件複製到此資料夾，這就是上述連結的來源。目前，由於我無法使用
Mac 的作業系統，因此需要 Mac 使用者的協助，提供 TinyTeX
及其套件之壓縮檔的連結。
...
\caption{\label{tab:mtcar}Dynamic variable in caption: 2018-11-12 11:29:13.}
\centering
